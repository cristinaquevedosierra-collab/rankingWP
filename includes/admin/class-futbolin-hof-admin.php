...
class Futbolin_HOF_Admin {
    ...
    if ($hof_debug) { error_log('HALL OF FAME DEBUG: Proceso AJAX iniciado.'); }

        if (!current_user_can('manage_options')) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Permisos insuficientes.'); }
        ...
        if (!$nonce_ok) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: ¡Fallo en la verificación de seguridad!'); }
            wp_send_json_error(['message' => __('Error de seguridad', 'ranking-futbolin')], 400);
        }
        if ($hof_debug) { error_log('HALL OF FAME DEBUG: Nonce de seguridad verificado.'); }

        if (!class_exists('Futbolin_API_Client') || !class_exists('Futbolin_Stats_Processor')) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Componentes no disponibles (API/Processor).'); }
            wp_send_json_error(['message' => __('Componentes no disponibles.', 'ranking-futbolin')], 500);
        }

        if ($hof_debug) { error_log('HALL OF FAME DEBUG: Obteniendo datos de la API/Base de Datos...'); }

        try {
            $client = new Futbolin_API_Client();
            $proc   = new Futbolin_Stats_Processor($client);

            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Iniciando bucle de procesamiento.'); }
            $items = $proc->build_hof_dataset(); // aplica filtro de modalidades + >=100 victorias

            $count = is_array($items) ? count($items) : 0;
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Cálculo finalizado. Se encontraron ' . $count . ' jugadores para el ranking.'); }
            ...
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Enviando respuesta JSON de éxito al navegador.'); }
            ...
            // En error sí dejamos un log visible
            error_log('HALL OF FAME ERROR: Excepción durante el cálculo -> ' . $e->getMessage());
    ...
    @set_time_limit(60);
    $hof_debug = (isset($_GET['rf_debug_hof']) && $_GET['rf_debug_hof'] == '1');
    if ($hof_debug) { error_log('HALL OF FAME DEBUG: Consulta de estado iniciada.'); }

        if (!current_user_can('manage_options')) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Permisos insuficientes en status.'); }
            wp_send_json_error(['message' => __('Permisos insuficientes.', 'ranking-futbolin')], 403);
        }
        $nonce_ok = check_ajax_referer('futbolin_admin_nonce', 'security', false);
        if (!$nonce_ok) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: ¡Fallo nonce en status!'); }
            wp_send_json_error(['message' => __('Error de seguridad', 'ranking-futbolin')], 400);
        }
        if ($hof_debug) { error_log('HALL OF FAME DEBUG: Nonce de seguridad verificado (status).'); }
    ...
    @set_time_limit(60);
    $hof_debug = (isset($_GET['rf_debug_hof']) && $_GET['rf_debug_hof'] == '1');
    if ($hof_debug) { error_log('HALL OF FAME DEBUG: Cancelación solicitada.'); }

        if (!current_user_can('manage_options')) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: Permisos insuficientes en cancelación.'); }
            wp_send_json_error(['message' => __('Permisos insuficientes.', 'ranking-futbolin')], 403);
        }
        $nonce_ok = check_ajax_referer('futbolin_admin_nonce', 'security', false);
        if (!$nonce_ok) {
            if ($hof_debug) { error_log('HALL OF FAME DEBUG: ¡Fallo nonce en cancelación!'); }
            wp_send_json_error(['message' => __('Error de seguridad', 'ranking-futbolin')], 400);
        }
    ...
    if ($hof_debug) { error_log('HALL OF FAME DEBUG: Transients/estado borrados. Enviando respuesta JSON de éxito.'); }
