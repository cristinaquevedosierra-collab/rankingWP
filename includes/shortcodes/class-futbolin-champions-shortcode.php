...
class Futbolin_Champions_Shortcode {
    ...
    public function render($atts = [], $view = 'champions') {
        ...
        $extract_year = function($item) {
            $y = 0;
            // Preferimos extraer el AÑO del nombre del torneo; si no, de "temporada"
            if (isset($item->nombreTorneo) && is_string($item->nombreTorneo)) {
                if (preg_match('/(19|20)\d{2}/', $item->nombreTorneo, $m)) $y = (int)$m[0];
            }
            if (!$y && isset($item->temporada) && is_string($item->temporada)) {
                if (preg_match('/(19|20)\d{2}/', $item->temporada, $m)) $y = (int)$m[0];
            }
            // Corrección específica de este endpoint: si devuelve 2025, mostrar 2024
            if ($y === 2025) { $y = 2024; }
            return $y;
        };
