...
<div class="futbolin-card">
  ...
  <?php else: ?>
    ...
    <?php foreach ($competitions as $competicion_name => $results):
        // Generar ID de ancla estable por competición: preferimos competicionId, si no slug del nombre
        $anchor_id = '';
        if (is_array($results)) {
          foreach ($results as $__r) {
            if (isset($__r->competicionId) && (int)$__r->competicionId > 0) { $anchor_id = 'comp-' . (int)$__r->competicionId; break; }
          }
        }
        if ($anchor_id === '' || !is_string($anchor_id)) {
          $slug = sanitize_title($competicion_name);
          if ($slug === '') { $slug = 'comp'; }
          $anchor_id = 'comp-' . $slug;
        }
      ?>
  <section class="tournament-competicion-block">
        <span id="<?php echo esc_attr($anchor_id); ?>" style="display:block; position:relative; top:-72px; height:0; visibility:hidden;"></span>
