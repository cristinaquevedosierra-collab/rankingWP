...
$tournaments_played        = $stats['tournaments_played']       ?? null;
...
<div class="futbolin-card">
    <h3 class="history-main-title">General</h3>
    <h3 class="section-title section-pro">Resultados en modalidades profesionales</h3>
    <?php
    $prof = isset($processor->professional_stats) && is_array($processor->professional_stats) ? $processor->professional_stats : [];
    $prof_total_matches       = $prof['total_matches']        ?? null;
    $prof_wins                = $prof['wins']                 ?? null;
    $prof_win_rate            = $prof['win_rate']             ?? null;
    $prof_total_competitions  = $prof['total_competitions']   ?? null;
    $prof_titles              = $prof['titles']               ?? null;
    $prof_competition_wr      = $prof['competition_win_rate'] ?? null;
    ?>
    <div class="player-career-stats-grid pro-emphasis">
        <div class="stat-box">
            <div class="stat-value"><?php echo $hn($prof_total_matches); ?></div>
            <div class="stat-label">Partidos Jugados</div>
        </div>
        <div class="stat-box">
            <div class="stat-value"><?php echo $hn($prof_wins); ?></div>
            <div class="stat-label">Partidos Ganados</div>
        </div>
        <div class="stat-box">
            <div class="stat-value"><?php echo $hperc($prof_win_rate); ?></div>
            <div class="stat-label">% Victorias (Partidos)</div>
        </div>
        <div class="stat-box">
            <div class="stat-value"><?php echo $hn($prof_total_competitions); ?></div>
            <div class="stat-label">Competiciones Jugadas</div>
        </div>
        <div class="stat-box">
            <div class="stat-value"><?php echo $hn($prof_titles); ?></div>
            <div class="stat-label">Títulos Ganados</div>
        </div>
        <div class="stat-box">
            <div class="stat-value"><?php echo $hperc($prof_competition_wr); ?></div>
            <div class="stat-label">% Victorias (Competiciones)</div>
        </div>
    </div>

    
    <p class="pro-disclaimer"><small>Estas estadísticas se basan en tus resultados en las modalidades de Open dobles, Open individual, Pro dobles, España dobles, España individual y Mixto, sin tener en cuenta los partidos de liguilla.</small></p>
    <hr style="border-top: 1px solid var(--futbolin-border-color); margin: 25px 0;">
    
    <h3 class="section-title section-global muted">Resultados Globales</h3>
    
    <div class="player-career-stats-grid global-muted">
        <div class="stat-box">
    ...
    <ul class="stats-list">
        ...
        <li>
            ...
            <span class="value"><?php echo $hn($tournaments_played); ?></span>
        </li>
    </ul>
