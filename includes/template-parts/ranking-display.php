 ...
 * ======================================================================*/
  ...
  // Solo mostrar Categoría en Individual/Dobles y nunca en vista anual
  $show_category = (in_array($mod_id, [1,2], true) && ($current_view !== 'annual'));
  ...
  // 3) Título
  $title_label = (isset($current_view) && $current_view === 'annual') ? 'Ranking anual' : 'Ranking';
  ...
  <div class="futbolin-card<?php echo $card_extra_class; ?>">
    <?php if (!empty($page_items)): ?>
        ...
        <div class="ranking-controls-right">
          <form class="futbolin-search-form" onsubmit="return false;">
            <div class="search-wrapper">
              <input type="text" class="futbolin-live-filter rf-search-input" placeholder="Escribe para filtrar…" autocomplete="off" aria-label="Filtrar jugadores" />
              <div class="rf-filter-meta" style="display:none"><span class="rf-filter-count" data-total="<?php echo (int)$totalCount; ?>">0</span> / <?php echo (int)$totalCount; ?> visibles</div>
            </div>
          </form>
          <div class="page-size-form">
            <?php
              ...
              foreach ([25=>'25', 50=>'50', 100=>'100', -1=>'Todos'] as $val => $label):
                ...
                $url = _futb_build_url_view($current_view ?: 'ranking', ['pageSize'=>$val, 'fpage'=>1, 'modalidad'=>$mod_id ?: null]);
            ?>
              <a class="button<?php echo $isActive ? ' active' : ''; ?>" href="<?php echo $url; ?>"><?php echo esc_html($label); ?></a>
            <?php endforeach; ?>
          </div>
      ...
      <div class="futbolin-paginacion" role="navigation" aria-label="Paginación ranking (top HECTOR)">
          <?php if ($page > 1): ?>
            <a class="button prev" href="<?php echo _futb_build_url_view($current_view ?: 'ranking', ['fpage'=>$page-1, 'modalidad'=>$mod_id ?: null, 'pageSize'=>$page_size]); ?>">← Anterior</a>
          <?php endif; ?>
          <span class="page-indicator">Página <?php echo (int)$page; ?> de <?php echo (int)$totalPages; ?></span>
          <?php if ($page < $totalPages): ?>
            <a class="button next" href="<?php echo _futb_build_url_view($current_view ?: 'ranking', ['fpage'=>$page+1, 'modalidad'=>$mod_id ?: null, 'pageSize'=>$page_size]); ?>">Siguiente →</a>
          <?php endif; ?>
        </div>
      ...
      <script>
      // === Filtro en vivo (client-side) con normalización de acentos ===
      (function(){
        const input = document.currentScript.parentNode.querySelector('.futbolin-live-filter');
        const rows  = document.currentScript.parentNode.querySelectorAll('#ranking-rows .ranking-row');
        const metaBox = document.currentScript.parentNode.querySelector('.rf-filter-meta');
        ...
        if(metaBox){
          ...
          metaBox.querySelector('.rf-filter-count').textContent = total;
          metaBox.querySelector('.rf-filter-count').setAttribute('data-total', total);
        }

        function normalize(str){
          return (str||'').toString().normalize('NFD').replace(/\p{Diacritic}+/gu,'').toLowerCase();
        }
        const cache = Array.from(rows).map(r=>{
          const nameEl = r.querySelector('.ranking-player-details');
          const txt = nameEl ? nameEl.textContent : r.getAttribute('data-player') || '';
        ...
        input.addEventListener('input', function(){
          ...
          if(val === last) return; // no cambia
          last = val;
          if(tId) cancelAnimationFrame(tId);
          tId = requestAnimationFrame(()=>{
            let visible = 0;
            cache.forEach((norm,i)=>{
              const show = !val || norm.indexOf(val) !== -1;
              rows[i].style.display = show ? '' : 'none';
              if(show) visible++;
            });
            if(metaBox){
              metaBox.querySelector('.rf-filter-count').textContent = visible;
            }
          });
        });
      })();
