...
class Futbolin_Reports_Engine {
    ...
    private function normalize_team($s): array {
        // Si llega como array estructurado, extraer nombres normalizados
        if (is_array($s)) {
            $norm = Futbolin_Normalizer::normalize_players($s);
            ...
            // Fallback a representación legible
            $teamStr = Futbolin_Normalizer::smart_team_to_string($s);
        } else {
            $teamStr = is_string($s) ? $s : (string)$s;
        }
        $teamStr = trim((string)$teamStr);
        $parts = preg_split('/\s+–\s+/u', $teamStr) ?: [$teamStr];
        if (count($parts) === 1) {
            $parts = preg_split('/\s*(?:—|–|-|\/|&|\s+y\s+)\s*/u', $teamStr) ?: [$teamStr];
        }
