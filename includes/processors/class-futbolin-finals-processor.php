...
class Futbolin_Finals_Processor {
    ...
    /** Separa y normaliza nombres de un equipo en jugadores. Acepta string o array. */
    private function split_players($team): array {
        // Si viene como array estructurado, extraer nombres directos
        if (is_array($team)) {
            $norm = Futbolin_Normalizer::normalize_players($team);
            ...
            // Si no hay nombres, convertir a string legible
            $teamStr = Futbolin_Normalizer::smart_team_to_string($team);
        } else {
            $teamStr = is_string($team) ? $team : (string)$team;
        }

        $normalized = trim((string)$teamStr);
    ...
    public function process_tournament_positions($positions, $reports) {
        ...
        foreach ($positions as $result) {
            $players = $this->split_players($result->equipoJugadores);
