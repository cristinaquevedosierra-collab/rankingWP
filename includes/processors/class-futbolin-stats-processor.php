...
class Futbolin_Stats_Processor {
    ...
    public function get_player_hall_of_fame_stats($player_id) {
        ...
        if (is_wp_error($player_data) || !$player_data || empty($player_data->nombreJugador)) {
            error_log('HALL OF FAME DEBUG: Datos jugador vacíos para ID=' . $player_id);
            return null;
        }
    ...
    public function build_hof_dataset(): array {
        error_log('HALL OF FAME DEBUG: build_hof_dataset() iniciado.');

        error_log('HALL OF FAME DEBUG: Derivando player_ids desde rankings...');
        $player_ids = $this->derive_player_ids_from_rankings();
        $player_ids = array_values(array_unique(array_map('intval', $player_ids)));
        error_log('HALL OF FAME DEBUG: player_ids derivados: ' . count($player_ids));
        ...
        foreach ($player_ids as $pid) {
            $i++;
            if ($i % 25 === 0) {
                error_log('HALL OF FAME DEBUG: Procesando jugador #' . $i . ' (ID=' . (int)$pid . ')');
            }
        ...
        error_log('HALL OF FAME DEBUG: Cálculo finalizado. Se encontraron ' . count($rows) . ' jugadores para el ranking.');
